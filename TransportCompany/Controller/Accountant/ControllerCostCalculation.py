...
class ControllerCostCalculation:
    def __init__(self, parent):
        self.__view = ViewCostCalculation()
        self.__model = ModelCostCalculation()
        self.__parent = parent
        self.__SettingsUI()
        self.__CostCalculation.closeEvent = self.__closeEvent

        self.__view.radioButton_DangerousCargo.clicked.connect(self.__ClickedDangerousCargo)
        self.__view.pushButton_Back.clicked.connect(self.__ClickedBack)
        self.__view.pushButton_Confirm.clicked.connect(self.__ClickedConfirm)
    ...
    def __FillingFieldsDriver1(self, InfoDriver):
        self.__view.lineEdit_FirstName.setText(InfoDriver[1])
        self.__view.lineEdit_LastName.setText(InfoDriver[2])
        self.__view.lineEdit_Patronymic.setText(InfoDriver[3])
        self.__view.lineEdit_Email.setText(InfoDriver[4])
        self.__view.lineEdit_Experience.setText(str(InfoDriver[5]))
        ProfitDriver1 = self.__CalculationProfitDriver(InfoDriver[5], self.CountDriver)
        self.__view.lineEdit_ProfitDriver.setText(ProfitDriver1)

        self.ProfitDriver1 = ProfitDriver1

    def __FillingFieldsDriver2(self, InfoDriver):
        self.__view.lineEdit_FirstName2.setText(InfoDriver[1])
        self.__view.lineEdit_LastName2.setText(InfoDriver[2])
        self.__view.lineEdit_Patronymic2.setText(InfoDriver[3])
        self.__view.lineEdit_Email2.setText(InfoDriver[4])
        self.__view.lineEdit_Experience2.setText(str(InfoDriver[5]))
        ProfitDriver2 = self.__CalculationProfitDriver(InfoDriver[5], self.CountDriver)
        self.__view.lineEdit_ProfitDriver2.setText(ProfitDriver2)

        self.ProfitDriver2 = ProfitDriver2

    def __CalculationProfitDriver(self, experience, CountDriver):
        IncomePerTrip = self.__view.lineEdit_ProfitCompany.text()
        try:
    ...
    def __FillingFieldDistance(self, StartLocation, EndLocation):
        ...
        self.__view.lineEdit_Distance.setText(str(distance))
    ...
    def __FillingFieldProfitCompany(self, wight):
        try:
            distance = float(self.__view.lineEdit_Distance.text())
            profit = round(CostTrip().Calcilating(distance, int(wight)), 2)
            self.__view.lineEdit_ProfitCompany.setText(str(profit))
        except ValueError:
            self.__view.lineEdit_ProfitCompany.setText("Доход не определен.Один из адресов некорректен")
            self.__view.pushButton_Confirm.setEnabled(False)

    def __ClickedDangerousCargo(self):
        try:
            if self.__view.radioButton_DangerousCargo.isChecked():
                self.__NewProfit(self.__view.lineEdit_ProfitDriver)
                if self.CountDriver == 2:
                    self.__NewProfit(self.__view.lineEdit_ProfitDriver2)
            else:
                self.__view.lineEdit_ProfitDriver.setText(self.ProfitDriver1)
                if self.CountDriver == 2:
                    self.__view.lineEdit_ProfitDriver2.setText(self.ProfitDriver2)
        except ValueError:
            pass

    def __NewProfit(self, lineEdit):
        IncomePerTrip = float(self.__view.lineEdit_ProfitCompany.text())
        ...
        try:
            if self.CountDriver == 2:
                self.__model.UpdateRevenue(self.InfoOrderAndDriver[0][0], self.id_order,
                                           float(self.__view.lineEdit_ProfitDriver.text()))
                self.__model.UpdateRevenue(self.InfoOrderAndDriver[1][0], self.id_order,
                                           float(self.__view.lineEdit_ProfitDriver2.text()))
            elif self.CountDriver == 1:
                self.__model.UpdateRevenue(self.InfoOrderAndDriver[0][0], self.id_order,
                                           float(self.__view.lineEdit_ProfitDriver.text()))
            self.__view.message("Информация", "Доход за поездку успешно назначен водителям")
            row = self.__parent.__view.tableWidget_TableApplication.currentRow()
            self.__parent.__view.tableWidget_TableApplication.removeRow(row)
            self.__parent.__view.lcdNumber.display(self.__parent.__view.lcdNumber.value() - 1)
